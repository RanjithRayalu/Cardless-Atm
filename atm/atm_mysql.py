...
class StartPage(tk.Frame):
    ...
    def _init_(self, parent, controller):
        tk.Frame._init_(self, parent,bg='#3d3d5c')
        self.controller = controller
        self.controller.title('CBIT')
        self.controller.state('zoomed')
        self.controller.iconphoto(False,tk.PhotoImage(file='abc.png'))

        heading=tk.Label(self,text='CBIT',font=('orbitron',45,'bold'),foreground='white',background='#3d3d5c')
        heading.pack(pady=25)

        space_label=tk.Label(self,height=4,bg='#3d3d5c').pack()

        password_label=tk.Label(self,text=(f'Welcome {user_display_name} to CBIT Banking'),font=('BatmanForeverAlternate',17),bg='#3d3d5c',fg='white').pack(pady=10)
        ...
        credits=tk.Label(bottom_frame,text='DO NOT SHARE YOUR ACCOUNT DETAILS',font=('orbitron',15)).pack()
...
class MenuPage(tk.Frame):
    ...
    def _init_(self, parent, controller):
        ...
        heading=tk.Label(self,text='CBIT ATM',font=('orbitron',45,'bold'),foreground='white',background='#3d3d5c')
        heading.pack(pady=25)
    ...
    def _init_(self, parent, controller):
        ...
        def withdraw(amount):
            ...
            if amount>current_balance:
                ...
                mydb=mysql.connector.connect(host="localhost",user="root",password="Ranjith@24")
                mycursor=mydb.cursor()
                mycursor.execute("use CBIT")
                mycursor.execute(f"update cbitbank set balance ={current_balance} where accid = {username1} ")
                mydb.commit()
        ...
        twenty_button=tk.Button(button_frame,text='₹200',font=('orbitron',12),command=lambda:withdraw(200),relief='raised',borderwidth=3,width=30,height=4)
        twenty_button.grid(row=0,column=0,pady=5)

        fourty_button=tk.Button(button_frame,text='₹400',font=('orbitron',12),command=lambda:withdraw(400),relief='raised',borderwidth=3,width=30,height=4)
        fourty_button.grid(row=1,column=0,pady=5)

        sixty_button=tk.Button(button_frame,text='₹600',font=('orbitron',12),command=lambda:withdraw(600),relief='raised',borderwidth=3,width=30,height=4)
        sixty_button.grid(row=2,column=0,pady=5)

        eighty_button=tk.Button(button_frame,text='₹800',font=('orbitron',12),command=lambda:withdraw(800),relief='raised',borderwidth=3,width=30,height=4)
        eighty_button.grid(row=3,column=0,pady=5)

        one_hundred_button=tk.Button(button_frame,text='₹1000',font=('orbitron',12),command=lambda:withdraw(1000),relief='raised',borderwidth=3,width=30,height=4)
        one_hundred_button.grid(row=0,column=1,pady=5,padx=794)

        two_hundred_button=tk.Button(button_frame,text='₹2000',font=('orbitron',12),command=lambda:withdraw(2000),relief='raised',borderwidth=3,width=30,height=4)
...
class InfoPage(tk.Frame):
    ...
    def _init_(self, parent, controller):
        ...
        mydb=mysql.connector.connect(host="localhost",user="root",password="Ranjith@24")
        mycursor=mydb.cursor()
        mycursor.execute("create database if not exists CBIT")
        mycursor.execute("use CBIT")
        mycursor.execute(f"select password from cbitbank where accid = {username1} ")
...
def about():
  ...
  Label(screen3,bg='lightblue', text = "Created and Developed by Ranjith,Vinay\n sem3 mini project ",font = ("orbitron", 10,'bold')).pack()
...
def register_user():
  ...
  mycursor.execute("create database if not exists CBIT")
  mycursor.execute("use CBIT")
  mycursor.execute("create table if not exists cbitbank(accid int(10) primary key,name varchar(30),password char(20),balance char(30))")
  mydb.commit()

  mycursor.execute('select accid from cbitbank')
  ...
  if username_info in b:
    ...
    name_entry.delete(0,END)
        ...
        name_entry.delete(0, END)
        screen1.destroy()
        mycursor.execute("insert into cbitbank values('"+username_info+"','"+name_info+"','"+password_info+"','"+balance_inti+"')")
        mydb.commit()
      ...
      if username1.isdigit():
          ...
          if int(username1) in user_acc:
              ...
              if password1=='':
                  ...
                  mycursor.execute(f"select name from cbitbank where accid={username1}")
                  values=mycursor.fetchall()
                  user_name=[]
                  for i in values:
                  ...
                  mycursor.execute(f'select balance from cbitbank where accid ={username1}')
                  values=mycursor.fetchall()
                  user_balance=[]
                  for i in values:
...
def main_screen():
  ...
  Label(text = "CBIT",fg='white', bg = "grey", width = "300", height = "2", font = ("orbitron", 15,'bold')).pack()
  Label(text = "",bg='lightblue').pack()

  img = ImageTk.PhotoImage(Image.open("images.jpeg"))
